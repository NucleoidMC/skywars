...
public class SkyWarsMapGenerator {
    ...
    public SkyWarsMap build(MinecraftServer server) throws GameOpenException {
        try {
            ...
            List<Vec3> spawns = template.getMetadata().getRegionBounds("spawn").map(bounds -> {
                Vec3 spawn = bounds.center();
                spawn = spawn.subtract(0, (bounds.max().getY() - bounds.min().getY() + 1) / 2.0D, 0);
                return spawn;
            }).collect(Collectors.toList());
            if (spawns.isEmpty()) {
                throw new GameOpenException(Component.literal("No player spawns defined."));
            }
